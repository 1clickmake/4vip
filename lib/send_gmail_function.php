...
function send_gmail($to_name, $to_email, $subject, $body,  $alt_body = '', $attachments = [], $cc_list = [], $bcc_list = [], $reply_to = []) {
    ...
    $gmail_account = 'wjw708@gmail.com'; // Gmail 계정
    $gmail_password = 'zprf nwrs vkfi japi'; // Gmail 앱 비밀번호 (보안을 위해 앱 비밀번호 사용)
    $sender_name = 'cmake'; // 발신자 이름
    ...
    try {
        // 서버 설정
        $mail->SMTPDebug = 2; // 디버그 출력 레벨 (0=비활성화, 1=메시지, 2=메시지+연결 상태)
        $mail->isSMTP(); // SMTP 사용
