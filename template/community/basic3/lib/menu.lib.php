...
function generateMegaMenu($parent_id = 0, $level = 1, $max_depth = 5, $ob_get = "") {
    global $pdo;
    ...
    // 메뉴 리스트 시작
    $ul_class = ($level == 1) ? 'nav-menu' : 'mega-menu-level-' . $level;
    ...
    foreach ($menus as $row) {
        ...
        // 메뉴 아이템 시작
        $li_class = ($sub_count > 0) ? '' : '';
        $output .= '<li class="' . $li_class . '">';

        if ($sub_count > 0) {
            // 드롭다운 메뉴
            $output .= '<a class="" href="' . $menu_url . '" id="megaMenuDropdown' . $menu_id . '" role="button" data-bs-toggle="dropdown" aria-expanded="false"' . $target . '>';
            $output .= $menu_name;
            $output .= '</a>';
