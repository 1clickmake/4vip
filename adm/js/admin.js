...
function deleteGroup(groupId, groupName) {
    if (confirm(`"${groupName}" 그룹을 정말 삭제하시겠습니까?`)) {
        fetch(cm_admin_url + '/board/board_group_form_update.php', {
            method: 'POST',
...
function editBoard(boardId) {
    fetch(cm_admin_url + '/ajax/get_board_data.php?board_id=' + encodeURIComponent(boardId))
        .then(response => response.json())
        .then(data => {
            if (data.error) {
                alert(data.error);
...
function deleteBoard(boardId, boardName) {
    if (confirm(`게시판 "${boardName}"을(를) 정말 삭제하시겠습니까?`)) {
        fetch(cm_admin_url + '/board/board_form_update.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
...
function deleteContent(id, co_id) {
    if (confirm(`정말로 "${co_id}" 내용을 삭제하시겠습니까?`)) {
        ...
        form.method = 'POST';
        form.action = cm_admin_url + '/board/content_form_update.php';
