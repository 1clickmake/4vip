...
<div class="main-content shifted" id="mainContent">
	<div class="container-fluid">
		...
		<form action="./config_form_update.php" method="post">
			...
			<div class="mb-3">
				<label for="ip_access" class="form-label">접근가능 IP (입력된 IP의 컴퓨터만 접근가능. 123.123.+ 도 입력 가능. (엔터로 구분))</label>
				<textarea class="form-control" id="ip_access" name="ip_access" rows="5"><?php echo $config['ip_access'] ?? '';?></textarea>
			</div>
			<div class="mb-3">
				<label for="ip_block" class="form-label">접근차단 IP (입력된 IP의 컴퓨터는 접근차단. 123.123.+ 도 입력 가능. (엔터로 구분))</label>
				<textarea class="form-control" id="ip_block" name="ip_block" rows="5"><?php echo $config['ip_block'] ?? '';?></textarea>
			</div>
			<div class="mb-3">
				<label for="recaptcha_site_key" class="form-label">구글 reCAPTCHA Site key(reCAPTCHA V3)</label>
				<input type="tel" class="form-control" id="recaptcha_site_key" name="recaptcha_site_key" value="<?php echo $config['recaptcha_site_key'] ?? '';?>" placeholder="">
			</div>
			<div class="mb-3">
