    ...
    <div class="card-body">
      ...
      <form id="loginForm" method="post">
        ...
        <div class="d-flex justify-content-between align-items-center mb-3">
          <a href="/forgot-password" class="text-decoration-none" style="color: #007bff; font-size: 0.9rem;">비밀번호를 잊으셨나요?</a>
        </div>
        <button type="submit" class="btn btn-primary w-100" style="border-radius: 10px; padding: 10px; font-weight: 500;">로그인</button>
      </form>
      <div class="text-center mt-3">
        <p class="mb-0" style="color: #777; font-size: 0.9rem;">계정이 없으신가요? <a href="/register" class="text-decoration-none" style="color: #007bff;">회원가입</a></p>
      </div>
    </div>
  </div>
</div>
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.status === 'success') {
                    window.location.href = response.redirect || '<?php echo CM_URL?>';
                } else {
