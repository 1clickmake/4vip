...
if (!$user_id || !$user_password) {
    die(json_encode(['status' => 'error', 'message' => '아이디와 비밀번호를 모두 입력해주세요.']));
}
...
if ($login_attempts >= 5 && (time() - $last_attempt_time) < 900) {
    die(json_encode(['status' => 'error', 'message' => '로그인 시도가 너무 많습니다. 15분 후에 다시 시도해주세요.']));
}
...
if ($member && password_verify($user_password, $member['user_password'])) {
   ...
   /*  $_SESSION['user_id'] = $member['user_id'];
    ...
    $_SESSION['user_no'] = $member['user_no']; */
